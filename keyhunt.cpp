		...
		if(OUTPUTSECONDS.IsGreater(&ZERO) ){
			...
			if(MPZAUX.IsZero()) {
				...
				if(pretotal.IsLower(&int_limits[0]))	{
					if(FLAGMATRIX)	{
						sprintf(buffer,"[+] \033[1;33mScan %s\033[0m | \033[1;33m%s Sec\033[0m | \033[1;33m%s keys/s\033[0m\n",str_total,str_seconds,str_pretotal);
					}
					else	{
						sprintf(buffer,"\r[+] \033[1;33mScan %s\033[0m | \033[1;33m%s Sec\033[0m | \033[1;33m%s keys/s\033[0m\r",str_total,str_seconds,str_pretotal);
					}
				}
				else	{
					i = 0;
					...
					if(FLAGMATRIX)	{
						sprintf(buffer,"[+] \033[1;33mScan %s\033[0m | \033[1;33m%s Sec\033[0m | \033[1;33m%s %s\033[0m | \033[1;33m%s\033[0m\n",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal);
					}
					else	{
						if(THREADOUTPUT == 1)	{
							sprintf(buffer,"\r[+] \033[1;33mScan %s\033[0m | \033[1;33m%s Sec\033[0m | \033[1;33m%s %s\033[0m | \033[1;33m%s\033[0m\r",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal);
						}
						else	{
							sprintf(buffer,"\r[+] \033[1;33mScan %s\033[0m | \033[1;33m%s Sec\033[0m | \033[1;33m%s %s\033[0m | \033[1;33m%s\033[0m\r",str_total,str_seconds,str_divpretotal,str_limits_prefixs[salir ? i : i-1],str_pretotal);
						}
